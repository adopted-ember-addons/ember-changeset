...
import {
  Changeset,
  EmberChangeset,
  Changeset as ChangesetFactory,
} from 'ember-changeset';
...
module('Unit | Utility | changeset', function (hooks) {
  ...
  test('#error returns the error object and keeps changes', async function (assert) {
    ...
    assert.deepEqual(
      classToObj(dummyChangeset.error),
      expectedResult,
    ...
    assert.deepEqual(
      dummyChangeset.change,
      { name: 'a' },
      'should return change object',
    );
    assert.deepEqual(
      dummyChangeset.get('change.name'),
      'a',
      'should return nested change',
    );
    assert.deepEqual(
      dummyChangeset.get('change').name,
      'a',
  ...
  test('#change works with arrays', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangeset.change,
      expectedResult,
      'should return new changes',
    );
    assert.deepEqual(
      dummyChangeset.exampleArray,
      expectedResult.exampleArray,
      'should return changes object',
    );
    assert.deepEqual(dummyChangeset.name, 'a', 'should return new value');
  ...
  test('data reads the changeset CONTENT', async function (assert) {
    ...
    assert.strictEqual(dummyChangeset.data, dummyModel, 'should return data');
  });
  ...
  test('data is readonly', async function (assert) {
    ...
    try {
      ...
      assert.throws(
        ({ message }) =>
          message ===
          "Cannot set read-only property 'data' on object: changeset:[object Object]",
        'should throw error',
      );
    }
  });
  ...
  test('changeset accepts changeset keys', async function (assert) {
    let dummyChangeset = Changeset(dummyModel, dummyValidator, null, {
      changesetKeys: ['name'],
    });
  ...
  test('changeset accepts changeset keys without validations', async function (assert) {
    let dummyChangeset = Changeset(dummyModel, null, null, {
      changesetKeys: ['name'],
    });
    dummyChangeset.set('_somePrivate', 'a');
  ...
  test('changeset isDirty property notifies dependent computed properties', async function (assert) {
    ...
    assert.notOk(model.hasDirtyChangesets, 'no dirty changesets');
    assert.strictEqual(
      model.dirtyChangesets.length,
      0,
      'has 0 dirty changesets',
    );
    assert.false(model.isChangesetDirty, 'changeset not dirty');

    let changesets = model.changesets;

    changesets[0].set('name', 'new name');
    assert.strictEqual(
      model.dirtyChangesets.length,
      1,
      'has one dirty changesets',
    );
    assert.ok(model.hasDirtyChangesets, 'has dirty changesets');

    let changeset = model.changeset;
    set(changeset, 'name', 'other new name');
    assert.true(model.isChangesetDirty, 'changeset is dirty');
  });
  ...
  test('#get proxies to content', async function (assert) {
    ...
    let dummyChangeset = Changeset(dummyModel);
    let result = dummyChangeset.name;
  ...
  test('#get returns the change when with properties added after', async function (assert) {
    ...
    assert.notOk(
      newValue._isUTC,
      'does not have changes from original instance',
    );
  });
  ...
  test('#get returns change that has array as sibling', async function (assert) {
    ...
    assert.strictEqual(
      dummyChangeset.name,
      'Burdger',
      'should proxy name to change',
    );
    assert.deepEqual(
      dummyChangeset.creds,
      ['burgers'],
      'should proxy creds to change',
    );
    ...
    assert.deepEqual(
      dummyChangeset.creds,
      ['fries'],
      'should proxy creds to change after change',
    );
  });
  ...
  test('nested objects can contain arrays', async function (assert) {
    ...
    assert.deepEqual(
      dummyModel.contact.emails,
      ['fred@email.com', 'the_fred@email.com'],
      'returns model saved value',
    );
  });
  ...
  test('#set Ember.set works for nested', async function (assert) {
    ...
    assert.deepEqual(
      changes,
      [{ key: 'name.short', value: 'boo' }],
      'changes with nested key Ember.set',
    );
  ...
  test('#set adds a change for a Date', async function (assert) {
    ...
    assert.strictEqual(
      dummyModel.dateOfBirth,
      undefined,
      'should not have value',
    );
    assert.deepEqual(
      dummyChangeset.get('dateOfBirth'),
      d,
      'should have new change',
    );
  ...
  test('#set does add a change if invalid', async function (assert) {
    ...
    let changes = dummyChangeset.changes;
    let errors = dummyChangeset.errors;
    let isValid = dummyChangeset.isValid;
    let isInvalid = dummyChangeset.isInvalid;
  ...
  test('#set operating on complex properties should be functional', async function (assert) {
    ...
    options = options.filter((o, i) =>
      i === options.indexOf('test2') ? false : true,
    );
    ...
    assert.deepEqual(changes, expectedChanges, 'should add change');
    assert.deepEqual(
      dummyChangeset.options,
      expectedChanges[0].value,
  ...
  test('#set Ember.set with Object actually does work TWICE for nested', async function (assert) {
    ...
    assert.strictEqual(
      get(dummyModel, 'name.title.id'),
      undefined,
      'should not have new change',
    );
    assert.strictEqual(
      dummyChangeset.name.title.id,
      'Mr',
      'should have new change',
    );
    assert.strictEqual(
      dummyChangeset.get('name.title.id'),
      'Mr',
      'should have new change using get',
    );

    let changes = dummyChangeset.changes;
    assert.deepEqual(
      changes,
      [{ key: 'name.title', value: title1 }],
      'changes with nested key Ember.set',
    );
  ...
  test('#set Ember.set with Ember Data Object actually does work TWICE for nested', async function (assert) {
    ...
    assert.strictEqual(
      dummyChangeset.profile.pet.breed,
      'jazzy',
      'should have change',
    );
    assert.strictEqual(
      dummyChangeset.get('profile.pet.breed'),
      'jazzy',
      'should have change using get',
    );
    assert.strictEqual(
      dummyChangeset.get('profile.pet').breed,
      'jazzy',
    ...
    assert.strictEqual(
      get(mockUserModel, 'profile.pet.breed'),
      'hands',
      'has new property',
    );
  });
  ...
  test('it works with setProperties', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.changes, expectedResult, 'precondition');
  });
  ...
  test('it clears errors when setting to original value', async function (assert) {
    ...
    assert.ok(dummyChangeset.isInvalid, 'should be invalid');
    dummyChangeset.set('name', 'Jim Bob');
    assert.ok(dummyChangeset.isValid, 'should be valid');
    assert.notOk(dummyChangeset.isInvalid, 'should be valid');
  });
  ...
  test('#set works when replacing an Object with an primitive', async function (assert) {
    ...
    assert.deepEqual(
      c.get('foo.bar.baz'),
      get(model, 'foo.bar.baz'),
      'model and changeset in sync',
    );

    c.set('foo', 'not an object anymore');
    c.execute();
    assert.deepEqual(c.get('foo'), model.foo);
  });
  ...
  test('#prepare provides callback to modify changes', async function (assert) {
    ...
    let changeKeys = dummyChangeset.changes.map((change) => change.key);

    assert.deepEqual(
      changeKeys,
      ['first-name', 'date-of-birth'],
      'should update changes',
    );
    dummyChangeset.execute();
    assert.strictEqual(
      get(dummyModel, 'first-name'),
      'foo',
      'should update changes',
    );
    assert.strictEqual(
      get(dummyModel, 'date-of-birth'),
      date,
      'should update changes',
    );
  });
  ...
  test('#prepare throws if callback does not return object', async function (assert) {
    ...
    try {
      ...
      assert.throws(
        ({ message }) =>
          message ===
          'Assertion Failed: Callback to `changeset.prepare` must return an object',
        'should throw error',
      );
    }
  });
  ...
  test('#execute applies changes to content if valid', async function (assert) {
    ...
    assert.strictEqual(dummyModel.name, undefined, 'precondition');
    assert.ok(dummyChangeset.isValid, 'should be valid');
    dummyChangeset.execute();
    assert.strictEqual(dummyModel.name, 'foo', 'should apply changes');
  });
  ...
  test('#execute does not apply changes to content if invalid', async function (assert) {
    ...
    assert.ok(dummyChangeset.isInvalid, 'should be invalid');
    dummyChangeset.execute();
    assert.strictEqual(dummyModel.name, undefined, 'should not apply changes');
  });
  ...
  test('#execute does not remove original nested objects', function (a) {
    ...
    let condition = dog.info instanceof DogTag;
  ...
  test('it works with nested keys', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangeset._content.org,
      expectedResult.org,
      'should have correct shape',
    );
    assert.deepEqual(dummyModel.org, expectedResult.org, 'should set value');
  });
  ...
  test('#pendingChanges up to date if changeset is invalid', async function (assert) {
    ...
    assert.strictEqual(dummyModel.name, undefined, 'precondition');
    assert.false(dummyChangeset.isValid, 'can be invalid');

    let pendingChanges = dummyChangeset.pendingData;

    assert.strictEqual(pendingChanges.name, 'a', 'should apply changes');
  });
  ...
  test('#pendingChanges up to date if changeset is valid', async function (assert) {
    ...
    assert.strictEqual(dummyModel.name, undefined, 'precondition');
    assert.true(dummyChangeset.isValid, 'can be valid');

    let pendingChanges = dummyChangeset.pendingData;

    assert.strictEqual(pendingChanges.name, 'foo', 'should apply changes');
  });
  ...
  test('#pendingChanges up to date after saved changeset modified again', async function (assert) {
    ...
    assert.strictEqual(
      dummyChangeset.pendingData.name,
      'foo',
    ...
    assert.strictEqual(
      dummyModel.name,
      undefined,
      'original data is not applied',
    );

    dummyChangeset.save();

    assert.strictEqual(dummyModel.name, 'foo', 'original data is updated');
    assert.deepEqual(
      dummyModel,
    ...
    assert.strictEqual(
      dummyChangeset.pendingData.name,
      'bar',
      'pending data is returned in pendingChanges',
    );
    assert.strictEqual(dummyModel.name, 'foo', 'original data is not modified');
  });
  ...
  test('#pendingChanges with ember-data model is not possible due to circular dependency when merging ember-data models', async function (assert) {
    ...
    assert.true(dummyChangeset.isPristine, 'changeset is pristine');

    dummyChangeset.set('profile.firstName', 'Zoe');

    assert.false(
      dummyChangeset.isPristine,
      'changeset is not pristine as there is a change',
    );
  ...
  todo(
    '#pendingChanges with ember-data model with multiple change steps',
    async function (assert) {
      ...
      let dummyChangeset = Changeset(mockUserModel);
      assert.equal(dummyChangeset.isPristine, true, 'changeset is pristine');

      dummyChangeset.set('profile.firstName', 'Zoe');

      assert.equal(
        get(mockUserModel, 'profile.firstName'),
        'Bob',
        'belongsTo property is not modified',
      );
      assert.equal(
        get(dummyChangeset, 'profile.firstName'),
        'Zoe',
        'belongsTo changeset is modified',
      );
      assert.equal(
      ...
      assert.equal(
        get(mockUserModel, 'profile.firstName'),
        'Zoe',
        'belongsTo property is updated after save',
      );
      assert.equal(
        get(dummyChangeset, 'profile.firstName'),
        'Zoe',
        'belongsTo changeset has new value',
      );
      assert.equal(
        get(dummyChangeset.pendingData, 'profile.firstName'),
        'Zoe',
        'pendingData should have a new value',
      );
      ...
      assert.equal(
        get(mockUserModel, 'profile.firstName'),
        'Zoe',
        'belongsTo property is model has initial value',
      );
      assert.equal(
        get(dummyChangeset, 'profile.firstName'),
        'Bob',
        'belongsTo changeset is updated',
      );
      assert.equal(
        get(dummyChangeset.pendingData, 'profile.firstName'),
        'Bob',
        'belongsTo pendingData has new value',
      );
      ...
      assert.equal(
        get(dummyChangeset.pendingData, 'profile.firstName'),
        'Bob',
        'belongsTo pendingData is updated',
      );
    },
  );
  ...
  test('#save proxies to content', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.change, { name: 'foo' }, 'should save');
    assert.strictEqual(
      options,
      'test options',
      'should proxy options when saving',
    );
    assert.strictEqual(
      typeof promise.then,
      'function',
    ...
    assert.strictEqual(
      saveResult,
      'saveResult',
      'save proxies to save promise of content',
    );
  });
  ...
  test('#save proxies to content even if it does not implement #save', async function (assert) {
    ...
    await dummyChangeset.save();
    assert.strictEqual(person.name, 'foo', 'persist changes to content');
  });
  ...
  test('calling save then setting a date from null to something and rejecting does return the right error', async function (assert) {
    ...
    const dummyModel = this.owner
      .lookup('service:store')
      .createRecord('profile', { startDate: null });
  ...
  test('#merge merges 2 valid changesets', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangesetC.changes,
      expectedChanges,
      'should merge 2 valid changesets',
    );
    assert.deepEqual(
      dummyChangesetA.changes,
      [{ key: 'firstName', value: 'Jim' }],
      'should not mutate first changeset',
    );
    assert.deepEqual(
      dummyChangesetB.changes,
      [{ key: 'lastName', value: 'Bob' }],
  ...
  test('#merge merges invalid changesets', async function (assert) {
    ...
    assert.true(
      dummyChangesetA.isInvalid,
      'changesetA is not valid becuase of name',
    );
    assert.true(dummyChangesetB.isValid, 'changesetB should be invalid');
    assert.true(dummyChangesetC.isInvalid, 'changesetC should be invalid');
    assert.true(dummyChangesetD.isInvalid, 'changesetD should be invalid');
    assert.deepEqual(
      dummyChangesetD.changes,
      expectedChanges,
      'should not merge invalid changes',
    );
    assert.deepEqual(
      dummyChangesetD.errors,
      expectedErrors,
      'should assign errors from both changesets',
    );
  });
  ...
  test('#merge does not merge a changeset with different content', async function (assert) {
    ...
    try {
      ...
      assert.throws(
        ({ message }) =>
          message ===
          'Assertion Failed: Cannot merge with a changeset of different content',
        'should throw error',
      );
    }
  });
  ...
  test('#merge preserves content and validator of origin changeset', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangesetC.get('errors'),
      expectedErrors,
      'should preserve validator',
    );
  ...
  test('#rollback restores old values', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.changes, expectedChanges, 'precondition');
    assert.deepEqual(dummyChangeset.errors, expectedErrors, 'precondition');
    dummyChangeset.rollback();
    assert.deepEqual(dummyChangeset.changes, [], 'should rollback');
    assert.deepEqual(dummyChangeset.errors, [], 'should rollback');
  });
  ...
  test('#rollback twice works', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.changes, expectedChanges, 'name is set');
    dummyChangeset.rollback();
    assert.deepEqual(dummyChangeset.changes, [], 'rolls back');
  ...
  test('#rollback twice with nested keys works', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.changes, expectedChanges, 'org is set');
    dummyChangeset.rollback();
    assert.deepEqual(dummyChangeset.changes, [], 'rolls back');
  ...
  test('#rollbackInvalid a specific key clears key error and keeps valid values', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.errors, expectedErrors, 'should rollback');
  });
  ...
  test('#rollbackInvalid will not remove changes that are valid', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangeset.changes,
      expectedChanges,
      'should not remove valid changes',
    );
    assert.ok(dummyChangeset.isValid, 'should still be valid');
  });
  ...
  test('#rollbackProperty restores old value for specified property only', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangeset.changes,
      expectedChanges,
      'should rollback single property',
    );
  });
  ...
  test('#validate works correctly with changeset values', async function (assert) {
    ...
    assert.strictEqual(
      get(dummyChangeset, 'errors.0.key'),
      'password',
      'has first error key',
    );
    assert.strictEqual(
      ...
      'org.isCompliant',
      'has second error key',
    );
    assert.strictEqual(
      get(dummyChangeset, 'errors.2.key'),
      'org.usa.ny',
      'has third error key',
    );
    assert.ok(dummyChangeset.isInvalid, 'should be invalid');
    ...
    assert.strictEqual(
      get(dummyChangeset, 'errors.3.key'),
      'passwordConfirmation',
    );
    assert.ok(dummyChangeset.isInvalid, 'should be invalid');
  ...
  test('#validate works correctly with complex values', async function (assert) {
    ...
    assert.deepEqual(dummyChangeset.changes[0], {
      key: 'options',
      value: { persist: true },
    });
  });
  ...
  test('#validate does not mark changes when nothing has changed', async function (assert) {
    ...
    await dummyChangeset.validate();
    assert.deepEqual(dummyChangeset.error, {});
    assert.deepEqual(dummyChangeset.changes, []);
  });
  ...
  test('#validate changeset getter', async function (assert) {
    ...
    assert.true(
      myChangeset.isInvalid,
      'Changeset is invalid as none of the options are selected',
    );
    ...
    assert.false(
      myChangeset.isInvalid,
      'Changeset is valid as one of the options is selected',
    );
  });
  ...
  test('#changeset properties do not proxy', async function (assert) {
    ...
    dummyChangeset.set('name', 'Jim Bob');
    await dummyChangeset.validate();
    assert.false(dummyChangeset.isValid, 'should have 1 error');
  });
  ...
  test('#removeErrors removes all errors', async function (assert) {
    ...
    assert.propEqual(dummyChangeset.errors, []);
  });
  ...
  test('#pushErrors updates error object in a KVO compatible way', function (assert) {
    ...
    assert.notOk(
      aComponentOrSomething.errorsOnName,
    ...
    assert.notOk(
      aComponentOrSomething.errorsOnName,
      'clearing of errors can be bound to',
    );
  });
  ...
  test('#restore restores a snapshot of the changeset', async function (assert) {
    ...
    assert.ok(dummyChangesetB.isValid, 'precondition - should be valid');
    dummyChangesetB.restore(snapshot);
    assert.ok(dummyChangesetB.isInvalid, 'should be invalid');
    assert.strictEqual(
      get(dummyChangesetB, 'change.name'),
      'Pokemon Go',
  ...
  test('#cast allows only specified keys to exist on the changeset', async function (assert) {
    ...
    assert.deepEqual(
      dummyChangeset.get('changes'),
      expectedResult,
      'should drop `unwantedProp',
    );
    assert.strictEqual(
      dummyChangeset.get('unwantedProp'),
      undefined,
      'should remove unwanted changes',
    );
  });
  ...
  test('can call with extended Changeset', async function (assert) {
    ...
    let dummyChangeset = ChangesetFactory(dummyModel, dummyValidator, null, {
      changeset: extended,
    });
    dummyChangeset.set('org.usa.ny', '');
